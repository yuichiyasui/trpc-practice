...
import type { TRPCMswConfig } from "./types";
...
const createTrpcHandler = (
  ...
  if (handlerType === "http") {
    if (procedureType === "query" || procedureType === "mutation") {
      const getInput =
        procedureType === "query" ? getQueryInput : getMutationInput;
      const httpHandler =
        procedureType === "mutation" || methodOverride === "POST"
          ? http.post
          : http.get;
      ...
      return httpHandler(urlRegex, async (params) => {
        try {
          ...
          return HttpResponse.json({
            result: { data: transformer.output.serialize(body) },
          });
        } catch (e) {
          ...
          return HttpResponse.json(
            { error: transformer.output.serialize(jsonError) },
            { status },
          );
        }
      });
